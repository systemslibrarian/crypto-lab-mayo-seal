...
Two are axe scans asserting zero WCAG 2.1 A/AA violations in **both** themes, across seven driven states per theme (after keygen for all five offered parameter sets, after the whipping walkthrough at toy and at real parameters, on an accepted signature, on each rejected one, under real parameters, after every forgery attempt and the malformed-input battery, and after a reference-vector replay), with every disclosure opened before each scan — an unscanned state is an ungated state.
