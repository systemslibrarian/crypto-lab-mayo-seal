 ...
 *    every verdict block is a `color-mix(in oklab, var(--ok|--alarm|--warn) 14%,
 *    var(--bg-2))`, every highlighted matrix cell a `color-mix(… 26%, …)`, and
 *    the hero aside a `color-mix(in oklab, var(--accent) 6%, transparent)`.
 *    That is where nearly every RESULT on the page is printed — the keygen
 *    readout, the three whipping beats, the VALID/REJECTED verdicts, every
 *    forgery attempt and both proof-lab panels — so a violations-only gate
 *    measured the contrast of almost nothing this lab computes.
 *  - text faded by an ancestor's `opacity`, or by a `filter`. Both appear here,
 *    and axe reads the declared `color` in either case.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.scroller` is
 *    this lab's own wrapper for every matrix it draws — the linear systems in
 *    Exhibit 2, the corner views at real parameters, the ledger tables — and at
 *    MAYO1 parameters those are 78 rows by 80 columns, so what is scrolled out
 *    of view is most of them. Content past the clip is not dimmed or partly
 *    drawn; it is absent from the frame, and asking what colour it sits on has
 *    no answer. Its rect is still outside every ancestor's box, so the walk
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. Every
 *    figure this lab draws — the whip figure, the matrices, the ledger bars —
 *    is HTML and CSS, and the page's only SVG is the shared header's two
 *    glyph-free `aria-hidden` marks. The branch is kept because the alternative
 *    is a helper that silently mismeasures the first labelled SVG figure anyone
 *    adds: an ancestor-only walk would composite a `<text>` onto the `<svg>`'s
 *    own transparent background rather than onto the `<rect>` drawn under it.
 *
 ...
 * not one — nor is it one to axe's `color-contrast` rule. `e2e/nontext.ts`
 * covers that gap with its own oracle and `scan()` asserts on it; this file
 * does not.
 */
 ...
 * uses. Nothing here is short-lived: every readout this lab produces is written
 * synchronously into a `role="status"` region and stays there until the next
 * button is pressed, and the stylesheet's only motion is
 * `transition-duration`, which the reduced-motion block this gate asserts
 * collapses to 0.001ms.
 *
 ...
 * sometimes-measured assertion is worse than none. The day this lab animates
 * anything, that is the tool for it.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page. The gradients here are the two `linear-gradient(45deg|135deg,
     * transparent 50%, currentColor 50%)` layers that draw the `<select>`
     * chevron — hard 50% bands whose "worst stop" is `currentColor` across the
     * whole control, which is not what any text on it sits on. So each gradient
     ...
     * unmeasurable and paints nothing. The radial branch is unexercised here
     * and kept for the same reason as the SVG branch above — the day a wash
     * appears, an unsampled gradient is a wrong number rather than a missing
     * one.
     */
     ...
     *
     * A driven pass here walks ten sections and — the expensive part — the
     * matrices, which are rendered one element per GF(16) nibble. A MAYO1
     * walkthrough draws a system 78 rows tall, so a single beat is thousands of
     * sibling cells all re-walking the same ancestors up to `<body>`. Without
     * the caches the pass re-reads the same computed styles and rects hundreds
     * of thousands of times. Nothing mutates the DOM during the pass, so the
     ...
     * that is `.scroller`, this lab's wrapper around every matrix and table it
     * draws, which is `overflow-x: auto` and at MAYO1 parameters holds a system
     * 78 rows tall. `.sr-only` is handled separately below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // itself — the whip figure, the matrices, the ledger bars — is HTML and
      // CSS.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // the two disclosures this page ships shut: the "Jargon, unpacked"
      // glossary and `#whip-full`, which is the only route to the five spec
      // operations behind Exhibit 2's three beats. The gate opens each by
      ...
      // before EVERY one of its scans, in the same helper that also stripped
      // `hidden` from every hidden element on the page.
      ...
      //
      // This lab uses the idiom for real, and for content that CHANGES, which
      // is why the guard is here and not merely inherited. `style.css` defines
      // `.sr-only` as `position:absolute; width:1px; height:1px;
      // clip:rect(0,0,0,0)`, and `dom.ts`'s `power()` emits one beside every
      // rendered superscript (16⁻³ paired with "16 to the power minus 3",
      // because screen readers announce Unicode superscripts inconsistently),
      // while `lesson.ts` appends " — done" / " — not yet" to each journey
      // stage so its state is not carried by the tick and the tint alone. Every
      // one has a rect and passes `checkVisibility()` while painting exactly
      // zero pixels; measuring them reports ratios for ink that was never laid
      // down, and the journey ones would report a fresh set on every step. It
     ...
     *
     * What this lab hides was enumerated rather than assumed, and every member
     * of the set is the REDUNDANT half of a pair the lab built deliberately:
     * the `.verdict__icon` (✓ ✗ ! ·) beside a worded headline in the same ink
     * on the same surface; the `.check__icon` in the forgery and precondition
     * batteries, likewise; the `.journey__tick` (✓ / ·), whose state is also
     * written into an `.sr-only` " — done" / " — not yet"; the `.swatch`
     * squares in the matrix legends, each followed by the words that name it
     * ("matches (thin underline)", "differs (struck through)"); and the visual
     * superscript in `power()`, whose spoken form sits in the `.sr-only` span
     * next to it.
     *
     * Nothing on this page hides a VALUE behind `aria-hidden`. No key, no
     * signature, no matrix entry and no verdict headline is inside such a
     ...
     *
     * This lab has no SVG of its own. The guard stays because the mistake it
     * catches is one element away from how this lab builds its DOM: `dom.ts`'s
     * `el(tag, attrs, children)` appends raw strings as text nodes wherever a
     * child array holds one, so a stray string in a `<g>`'s child list would
     * render nothing while a walk that measured it reported a 1:1 failure
     ...
     *
     * Here the root branch is the one that fires: this stylesheet sets
     * `background-color` on `html` in longhand (deliberately — axe and WebKit
     * miss the `background` shorthand), so `document.documentElement` really
     * does paint `--bg`. That makes the fallback below dormant rather than
     * unnecessary, and it is worth keeping for the reason it was written: this
     * page runs ten sections deep and a MAYO1 walkthrough adds several screens
     * more, so the day the declaration moves from `html` to `body` — a change
     * nobody would think of as risky — every element below the first screen
     * would otherwise report against a white page that does not exist.
     */
