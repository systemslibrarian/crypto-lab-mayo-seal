...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page. Here the live case is the `<select>` chevron, drawn as two
     * `linear-gradient(45deg|135deg, transparent 50%, currentColor 50%)` layers
     * whose hard 50% band covers only the right-hand corner of the control —
     * so the "worst stop" is `currentColor` and the real fill under most of the
     * box is the background colour. Each gradient is therefore *sampled* at the
     * element's real location: linear by projecting onto the gradient line,
    ...
    const CONTROL = [
      ...
      // itself as a control?" test below. Nothing inside `#app` matches it on
      // this page today; the jump navigation is a list of prose links.
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ].join(',');
    ...
    // repo in the fleet carries a copy inside `index.html` — and its `.cl-btn`
    // boundary is `color-mix(in srgb, var(--accent, #35d6bb) 38%, transparent)`
    // over the bar's fixed `#0b1512`, which measures the same in BOTH themes
    // because the bar is always dark and the page theme cannot move it. That is
    // a real 1.4.11 shortfall and it is reported upward as a fleet-wide
    ...
    // itself renders lives under `#app`, which on this page is the `<main
    // class="shell">`, and is judged.
